...
describe('resolveLevel', () => {
    ...
    test('should let a key naming the identifier win over a pattern that also matches', () => {
        const overrides: LogOverridesType = { '.*': 'error', 'direct-eval': 'verbose' };

        expect(resolveLevel(overrides, 'direct-eval', 'warning')).toBe('verbose');
        expect(resolveLevel(overrides, 'empty-glob', 'warning')).toBe('error');
    });

    test('should let the first pattern the table declared win', () => {
        const overrides: LogOverridesType = { 'direct-.*': 'verbose', '.*-eval': 'error' };
...
describe('collectLog', () => {
    ...
    test('should file a message under the level it was given', () => {
        ...
        expect(logs).toEqual({ verbose: [], info: [{ text: 'bundling' }], warning: [], error: [] });
    });
    ...
    test('should drop a message an override silenced', () => {
        ...
        expect(logs).toEqual({ verbose: [], info: [], warning: [], error: [] });
    });
...
describe('collectLogs', () => {
    ...
    test('should keep the order the messages arrived in', () => {
        collectLogs(logs, {}, [{ text: 'first' }, { text: 'second' }], 'verbose');

        expect(logs.verbose).toEqual([{ text: 'first' }, { text: 'second' }]);
    });
