    ...
    describe('configFileProvider', () => {
        ...
        describe('the build it runs the file through', () => {
            ...
            test('should register the map beside the code it built', async () => {
                ...
                expect(sourceMapMock).toHaveBeenCalledWith(path, 'map-text', true);
            });

            test('should register the map over whatever an earlier read left under the same name', async () => {
                await configFileProvider(path);
                await configFileProvider(path);

                expect(sourceMapMock).toHaveBeenNthCalledWith(2, path, 'map-text', true);
            });
