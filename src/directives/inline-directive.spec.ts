...
describe('evaluate', () => {
    ...
    test('should keep the map beside what was built under the name the run carries', async () => {
        ...
        expect(sourceMapMock).toHaveBeenCalledWith(inlineTarget, 'map', true);
    });

    test('should register the map over whatever an earlier run left under the same name', async () => {
        await evaluate(state, call);
        await evaluate(state, call);

        expect(sourceMapMock).toHaveBeenCalledTimes(2);
        expect(sourceMapMock).toHaveBeenNthCalledWith(2, inlineTarget, 'map', true);
    });
