...
describe('VariantService', () => {
    ...
    describe('the build it runs', () => {
        ...
        test('should hand back the result under the logs it collected', async () => {
            ...
            expect(await service.build()).toEqual({
                outputFiles: [ 'built' ], info: [], verbose: [], errors: [], warnings: []
            });
        });
    ...
    describe('the types it checks', () => {
        ...
        test('should hand back a bucket for every level, empty where it found nothing to report', async () => {
            expect(await service.check()).toEqual({ info: [], verbose: [], error: [], warning: [] });
        });
        ...
        test('should file each diagnostic under the level of its category', async () => {
            ...
            const logs = await service.check();
            ...
            expect(logs.warning.map((message: any) => message.id)).toEqual([ 'TS6133' ]);
            expect(logs.info.map((message: any) => message.text)).toEqual([ 'a note' ]);
            expect(logs.verbose.map((message: any) => message.text)).toEqual([ 'a plain message' ]);
        });

        test('should file a type error as an error whatever the configuration asks of a build', async () => {
            typescript.check.mockReturnValue([{ category: 1, code: 2304, message: 'an error' }]);
            configure({ types: { failOnError: false } });

            const logs = await service.check();

            expect(logs.error.map((message: any) => message.id)).toEqual([ 'TS2304' ]);
            expect(logs.warning).toEqual([]);
        });

        test('should file the diagnostics under the levels the configuration overrides', async () => {
            typescript.check.mockReturnValue([{ category: 1, code: 2304, message: 'an error' }]);
            configure({ logOverride: { 'TS2304': 'verbose' } });

            const logs = await service.check();

            expect(logs.error).toEqual([]);
            expect(logs.verbose.map((message: any) => message.id)).toEqual([ 'TS2304' ]);
        });
    ...
    describe('the plugin it registers', () => {
        ...
        test('should hand the hooks a context naming the variant', async () => {
            ...
            expect(context).toEqual(expect.objectContaining({
                variantName: 'esm',
                argv: { watch: true },
                logs: { info: [], verbose: [], error: [], warning: [] }
            }));
        });
    ...
    describe('the diagnostics it files', () => {
        test('should file each diagnostic under the level of its category', async () => {
            ...
            expect(context.logs.verbose).toEqual([
                {
