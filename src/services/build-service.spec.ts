...
describe('BuildService', () => {
    ...
    /**
     * The logs a check hands back, carrying one error where the variant reported anything.
     */

    function checked(name?: string): any {
        return {
            info: [],
            verbose: [],
            warning: [],
            error: name ? [{ text: `${ name } diagnostic`, pluginName: 'typescript' }] : []
        };
    }
    ...
    beforeEach(() => {
        ...
        checkMock = xJet.spyOn(VariantService.prototype, 'check').mockResolvedValue(<any> checked());
    ...
    describe('the builds it runs', () => {
        ...
        test('should leave a skipped variant carrying no output and nothing at any other level', async () => {
            ...
            expect(app.verbose).toEqual([]);
            expect(app.warnings).toEqual([]);
        });

        test('should carry one set of buckets between the result it shapes and the context it reports', async () => {
            failing('types');
            const listener = xJet.fn();
            declareVariants({ app: { dependOn: 'types' }, types: {} });
            service.subscribe(listener);
            ...
            const [ app ] = await service.build();
            const { logs } = listener.mock.calls[0][0].context;

            expect(app.info).toBe(logs.info);
            expect(app.errors).toBe(logs.error);
            expect(app.verbose).toBe(logs.verbose);
            expect(app.warnings).toBe(logs.warning);
        });

        test('should name every level on the result the context names on its logs', async () => {
            failing('types');
            const listener = xJet.fn();
            declareVariants({ app: { dependOn: 'types' }, types: {} });
            service.subscribe(listener);
            ...
            expect(Object.keys(logs).sort()).toEqual([ 'error', 'info', 'verbose', 'warning' ]);
            expect(Object.keys(app).sort()).toEqual([ 'errors', 'info', 'verbose', 'warnings' ]);
        });
        ...
        test('should shape a skipped variant\'s context from the configuration entry it holds', async () => {
            ...
            const listener = xJet.fn();
            declareVariants({
                app: { dependOn: 'types', logOverride: { 'TS2304': 'silent' } },
                types: {}
            });
            service.subscribe(listener);
    ...
    describe('the types it checks', () => {
        test('should check every variant and key the logs it filed by name', async () => {
            checkMock.mockImplementation(async function (this: any) {
                return <any> checked(this.name);
            });
        ...
        test('should hand back the buckets a variant that checked clean filed nothing in', async () => {
            declareVariants({ esm: {} });

            expect(await service.typeChack()).toEqual({
                esm: { info: [], verbose: [], warning: [], error: [] }
            });
        });
        ...
        test('should check the variants named and leave the rest alone', async () => {
            ...
            expect(await service.typeChack([ 'esm' ])).toEqual({ esm: checked() });
        ...
        test('should leave what a named variant waits for unchecked', async () => {
            ...
            expect(await service.typeChack([ 'app' ])).toEqual({ app: checked() });
        });
