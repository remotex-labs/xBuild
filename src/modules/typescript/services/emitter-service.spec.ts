...
describe('EmitterService', () => {
    ...
    describe('emit', () => {
        ...
        test('emits declaration file when version is new or changed', async () => {
            const mkdirSpy = xJet.mock(mkdir);
            const writeSpy = xJet.mock(writeFile);
    ...
    describe('alias path rewriting', () => {
        test('rewrites alias imports to relative paths in declaration content', async () => {
            const writeSpy = xJet.mock(writeFile);
            languageHostService.aliasRegex = new RegExp(
                '(?:^|\\s)(?:import|export)\\s+' +  // import or export keyword
                '(?:type\\s+)?' +                   // optional 'type' keyword
                '(?:[^\'"]*from\\s+)?' +            // optional '... from' (non-greedy)
                '[\'"]((@app/utils)[^\'"]*)[\'"]' +  // capture the quoted path with alias
                ';?',                               // optional semicolon
                'gm'
            );

            languageHostService.resolveModuleName = () => ({
                resolvedModule: { resolvedFileName: '/project/root/src/utils.ts' }
            });

            const emittedContent = `
                import { Util } from '@app/utils';
                export declare const helper: typeof Util;
            `;

            languageService.getEmitOutput = () => ({
                emitSkipped: false,
                outputFiles: [{ text: emittedContent }]
            }) as any;
            ...
            expect(written).not.toContain('@app/utils');
            expect(written).toContain('./utils');
        });
    });
});
