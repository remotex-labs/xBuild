...
export class EmitterService {
    ...
    private resolveModuleName(moduleName: string, containingFile: string): string | undefined {
        const resolved = this.languageHostService.resolveModuleName(
     ...
     *
     * @param aliasRegex - regex matching alias-based import/export statements
     * @param content - raw declaration text
     * @param sourceFile - source file context (for dirname and resolution base)
     * @returns content with aliases replaced by relative paths
     *
     * @remarks
     * Ensures emitted `.d.ts` files use portable relative imports instead of aliases.
     *
     * @since 2.0.0
     */

    private resolveAliases(aliasRegex: RegExp, content: string, sourceFile: SourceFile): string {
        return content.replace(aliasRegex, (match, importPath) => {
            const resolve = this.resolveModuleName(importPath, sourceFile.fileName);
    ...
    private async emitSingleDeclaration(sourceFile: SourceFile, options: CompilerOptions): Promise<void> {
        ...
        const aliasRegex = this.languageHostService.aliasRegex;
        if (aliasRegex) content = this.resolveAliases(aliasRegex, content, sourceFile);
